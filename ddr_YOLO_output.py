...
# added by Meng to specify CPU and set memory growth
os.environ["CUDA_VISIBLE_DEVICES"]="0" 
config_g = tf.ConfigProto()
config_g.gpu_options.allow_growth = True
session = tf.Session(config=config_g)
...
def do_predict(pred_func, input_file):
    ...
    viz = final # by Meng to remove original image in output
    # cv2.imwrite(input_file.replace("input", "output"), viz)
    
    # the following added by Meng to produce anno files
    # cv2.imwrite(input_file.replace("vis_papers", "vis_papers_frcnn"), viz) # note the output folder
    
####### save images with bounding boxes #######
    cur = input_file.rfind('/')
    cv2.imwrite(input_file[:cur] + "_frcnn" + input_file[cur:], viz) # note the output folder
####### save images with bounding boxes #######    
    if '.jpg' in input_file:
        imgtype = '.jpg'
    elif '.png' in input_file:
        imgtype = '.png'
    with open((input_file[:cur] + "_frcnn_anno" + input_file[cur:]).replace(imgtype, ".txt"), "w") as pred_f:
        for box in results:
            label = ""
            coor = ""
            for i in range(4):
                coor = coor + " " + str(int(box[0][i]))
            # label = label + str(round(box[1],2)) + " " + str(box[2]) + "\r"
            clsname = cfg.DATA.CLASS_NAMES[box[2]]
            label = label + clsname + " " + str(round(box[1],2)) + coor + "\r" # str(box[2]-1) or clsname
            # print("label is", label)
            pred_f.write(label)
    logger.info("Inference output for {} written to predict folder".format(input_file))
    # tpviz.interactive_imshow(viz)
...
if __name__ == '__main__':
    ...
    if args.visualize:
        ...
        ###### Meng - create the outpur folder
            image_path = args.predict[0]
            cur = image_path.rfind('/')
            os.makedirs(image_path[:cur] + "_frcnn", exist_ok=True)
            os.makedirs(image_path[:cur] + "_frcnn_anno", exist_ok=True)
        ######
